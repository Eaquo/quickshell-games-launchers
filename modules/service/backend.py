...
class GameLauncher:
    def __init__(self, config_path: str = None):
        ...
        self.image_cache = ImageCache(cache_dir / "image_cache.json", ttl_hours=cache_ttl)
    ...
    def get_default_config(self) -> Dict[str, Any]:
        return {
            ...
            "steamgriddb": {
                "enabled": False, "api_key": "", "image_type": "grid",
                "prefer_animated": False, "fallback_to_steam": True,
                "dimensions": [], "styles": [], "cache_ttl_hours": 24,
                "parallel_requests": True, "max_workers": 10, "request_timeout": 3
            },
            "heroic":     {"enabled": True, "config_paths": ["~/.config/heroic"],
                           "scan_epic": True, "scan_gog": True, "scan_amazon": True, "scan_sideload": True},
            "filtering":  {"games_only": False, "exclude_categories": [], "exclude_keywords": []},
    ...
    def should_include_game(self, game: Dict[str, Any]) -> bool:
        ...
        if filtering.get("games_only", False):
            ...
            if category in ["launcher", "desktop"]:
                if not (category == "desktop" and ("steam" in name or "heroic" in name or game.get("appid"))):
                    return False
            exclude_patterns = [
        ...
        if any(k.lower() in game_name_lower for k in filtering.get("exclude_keywords", [])):
            return False
        return True

    _SOURCE_PRIORITY = {
        "steam": 4, "epic": 3, "gog": 3, "amazon": 3,
        "heroic": 3, "lutris": 2, "desktop": 1, "manual": 5
    }
    ...
    def get_all_games(self) -> Dict[str, Any]:
        ...
        wallust_colors = self.load_wallust_colors() if self.config.get("appearance", {}).get("use_wallust", True) else {}
        state          = self.load_state()
        return {
            "games":       games,
            "config":      self.config,
